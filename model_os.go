...
func (v *OS) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := OS(value)
	...
	return fmt.Errorf("%+v is not a valid OS", value)
}

// NewOSFromValue returns a pointer to a valid OS
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewOSFromValue(v string) (*OS, error) {
	ev := OS(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for OS: valid values are %v", v, AllowedOSEnumValues)
	}
}
...
// Ptr returns reference to OS value
func (v OS) Ptr() *OS {
	return &v
}
